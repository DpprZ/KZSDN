...
<div>
    <div>
        <hr>
        <input  type="hidden" id="collect_id" name="collect_id" value="${blog.id}">
        <input id="username" name="username" value="${username}" type="hidden">
        <button style="background-color: yellow"  id="collectbt" name="collectbt" onclick="collect()" >收藏博客</button>
        <button style="background-color: red" id="favorite" name="favorite" onclick="favorite()">点赞(${blog.favorite})</button>
    </div>
...
<script>
    function favorite()
    {
        ...
        request.open("GET", "favoriteblog?username=" + username, true);
        request.send();
        window.location="ReadBlog1.jsp";
    }
    function collect() {
        var username=document.getElementById("username").value;
        var request = ajaxFunction(); //创建xmlHttpRequest对象
